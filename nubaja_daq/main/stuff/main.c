...
/* TODO: test oscillator w/ engine speed sensor for improved performance */
...
void control_dyno()
{
    ...
    if ( SENSOR_ENABLE )
    {
        ...
        if ( MPH_FLAG )
        {
            ...
            double v_car = (double) MPH_SCALE / (double) (period_speed_ms * CONTROL_LOOP_PERIOD) ;
            old_time = curr_time;
        ...
        if ( RPM_FLAG )
        {
            ...
            double RPM = (double) RPM_SCALE / (double) (period_RPM_ms * CONTROL_LOOP_PERIOD);
...
/*
 * Resets interrupt and calls control function to do actual work
 */
void control_thread()
{
...
void app_main()
{
    ...
    if (xSemaphoreTake(commsSemaphore, portMAX_DELAY) == pdTRUE)
    {
        ...
        xTaskCreatePinnedToCore(control_thread, "control", 2048, NULL, (configMAX_PRIORITIES-1), &ctrlHandle, 0);
