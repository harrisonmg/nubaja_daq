...
void AS1115_config () {
    ...
    i2c_write_byte(PORT_1, 0x0,0x2d,0x1); //enable self addressing setting the slave-addr to 0x03   
    i2c_write_byte(PORT_1, 0x0,0xc,0x81); //sets shutdown register for normal operation
    i2c_write_byte(PORT_1, 0x0,0xe,0x04); //sets features as desired with hex-code font
    i2c_write_byte(PORT_1, AS1115_SLAVE_ADDR,0x9,0xff); //decode mode enabled for all digits
    i2c_write_byte(PORT_1, AS1115_SLAVE_ADDR,0xa,0xee); //global intensity set to 15/16
    i2c_write_byte(PORT_1, AS1115_SLAVE_ADDR,0xb,0x3); //scan limit set to only display 4 digits 
    i2c_write_byte(PORT_1, AS1115_SLAVE_ADDR,DIGIT_3,0x5);
    i2c_write_byte(PORT_1, AS1115_SLAVE_ADDR,DIGIT_2,0xd);
    i2c_write_byte(PORT_1, AS1115_SLAVE_ADDR,DIGIT_1,0xa);
